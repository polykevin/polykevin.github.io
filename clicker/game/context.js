...
var projekt = {
  ...
  mechant: {// liste conteant tout les boutons du jeu
    test: {
...
function clique(truc) {// fonction clique qui permet d'acheter une amélioration
  if (projekt.rotule[truc].cout <= projekt.argent) {// si le cout de l'amélioration est inférieur ou égale a l'argent(vérifie si on a assez d'argent pour acheter)
...
function clique2(truc) {//fonction clique 2 fonctionne pareil que la fonction clique mais juste pour les boutons de la 2iéme fenetre
...
function jour() {// fonction jour qui est utilisé lorsque le site charge au début
  if (Cookies.get("projekt") != null && Cookies.get("projekt") != "undefined") {// vérifie les cookies
    var projekt1 = JSON.parse(Cookies.get("projekt"));
    for (i in projekt.rotule) {// rajoute toute les données des premier boutons dans les cookies
      if (projekt1.rotule[i] == null) {
        projekt1.rotule[i] = projekt.rotule[i];
      }
    }

    for (i in projekt.mechant) {// rajoute toutes les données des second boutons dans les cookies
  ...
  if (Cookies.get("lasttime") != null) {// vérifie les cookies actuel avec les anciens afin de déterminer ca fait combien de temps que la persoone a quitté le site
    ...
    if (lastsavedate / 60 >= 1) {// si la persoonne a quitté notre pendant plus d'une minute on calcule combien d'argent il s'est fait pendant son absence
      projekt.argent += lastsavedate * aps / 1.8;// on rajoute l'argent qu'il s'est fait
      alert("Vous avez fais " + numberformat.format(lastsavedate * aps / 1.8) + " $ " + "Pendant votre absence");// on crée un pop up qui indique au joueur combien d'argent il a pu crée
    }
  }

  setInterval(() => {// crée une intervalle ou dans l'aquelle tout les demi seconde il va executé ce code
    for (i in projekt.rotule) {// boucle dans chaque bouton
    ...
    if (aps < 100000) {//  ajoute le code html qui affiche le nombre d'argent par seconde on vérifie si c'est inférieur a 100000 car sinon on ne voit pas les virgules
    document.querySelector("#owo").innerHTML = aps + " $/Sec"; 
  }
    else {
    document.querySelector("#owo").innerHTML = numberformat.format(aps) + " $/Sec"; // ajoute le code html qui affiche le nombre d'argent par seconde arrondi si c'est supérieur à 100000
    }
    document.querySelector("#uwu").innerHTML = numberformat.format(Number(String(projekt.argent).split(".")[0])) + " $";// ajoute le code html qui affiche le nombre d'argent au total qu le joueur posséde et arrondi de sorte qu'il n'y ait pas de décimale
    delai++;// ajoute 1 au delai
    if (delai >= 40) {// si le delai est supérieur ou égale à 40 on met à jour les cookies et ainsi sauvegarde les donnée
...
function reset() {// fonction reset qui est utilisé par le bouton du même nom
  ...
  for (i in projekt.mechant) {// boucle dans chaque 2iéme bouton bouton
...
function autoclicker () {// fonction autoclicker utilisé à chaque clique sur le bouton principal
  for (i in projekt.mechant) {// boucle dans chaque dexiéme bouton
    if (projekt.mechant[i].nb > 0) {// si le nombre possédé du bouton est supérieur à 0
